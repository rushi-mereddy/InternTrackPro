...
import { formatDate, formatSalary, isDeadlineApproaching } from '@/lib/utils';
...
const JobDetail = () => {
  ...
  // Filter related courses based on job skills
  const relatedCourses = courses
    .filter((course: any) => {
      if (!job?.skillsRequired?.length) return false;
      const courseTitle = course.title.toLowerCase();
      return job.skillsRequired.some((skill: string) => 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 mb-6">
            ...
            {/* Description */}
            <div className="p-6">
              <h2 className="text-lg font-medium text-gray-900 mb-4">About the job</h2>
            ...
            <div className="p-6">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700">{isRemote ? 'Remote' : location}</span>
                </div>
                <div className="flex items-center">
                  <Briefcase className="h-5 w-5 text-gray-400 mr-3" />
                  <span className="text-gray-700">
                    {isFresherJob 
                      ...
                      : experienceRequiredYears === 0
                        ? 'No experience required'
                        : `${experienceRequiredYears}+ years of experience`}
                  </span>
                </div>
                <div className="flex items-center">
              ...
              {!user && (
                <div className="mt-4 p-3 bg-gray-50 rounded-md text-sm text-gray-600">
                  <p>Please <Link href="/login" className="text-primary-600 font-medium">log in</Link> or <Link href="/register" className="text-primary-600 font-medium">register</Link> to apply for this job.</p>
                </div>
              )}
            </div>
          </div>
